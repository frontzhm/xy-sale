...
import { createInbound, updateInbound, type InboundFormState } from "./actions";
...
export function InboundForm({
  ...
  const action = isEdit ? updateInbound : createInbound;
  const [state, formAction, pending] = useActionState<InboundFormState, FormData>(action, null);
  ...
  return (
    <form key={recordId ?? "new"} action={formAction} className="max-w-2xl space-y-8">
      ...
      <section className="space-y-4">
        <h2 className="text-sm font-medium text-zinc-500 dark:text-zinc-400">入库照片</h2>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-sm font-medium text-zinc-500 dark:text-zinc-400">入库明细</h2>
          <button
            type="button"
      ...
      <div className="flex flex-wrap gap-3 border-t border-zinc-200 pt-6 dark:border-zinc-800">
        ...
        >
          {pending ? "保存中…" : isEdit ? "保存修改" : "保存入库登记"}
        </button>
        <Link
          href={isEdit && recordId ? `/inbound/${recordId}` : "/inbound"}
