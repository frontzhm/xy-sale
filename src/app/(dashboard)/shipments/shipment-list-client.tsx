...
export function ShipmentListPageClient({
  manufacturers,
  catalog,
  initialQ,
  initialProductId,
  ...
  const columns = useMemo<ProColumns<ShipmentTableRow>[]>(
    () => [
      {
        title: "综合搜索",
        dataIndex: "searchQ",
  ...
  const clearHrefBase =
    initialQ.trim() !== ""
      ? `/shipments?q=${encodeURIComponent(initialQ.trim())}`
      : "/shipments";
      ...
      <div className="overflow-x-auto rounded-lg border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
        <ProTable<ShipmentTableRow>
          ...
          request={async (params) => {
            ...
            if (sq) sp.set("q", sq);
            const res = await fetch(`/api/shipments/table?${sp.toString()}`);
            if (!res.ok) {
              const text = await res.text();
              throw new Error(text || "加载失败");
            }
            const j = (await res.json()) as {
              data: ShipmentTableRow[];
              total: number;
              success?: boolean;
              filterMeta: ShipmentTableFilterMeta;
            };
